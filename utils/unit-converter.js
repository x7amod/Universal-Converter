...
window.UnitConverter.UnitConverter = class {
  constructor() {
    ...
    // Build unit-to-type lookup map for O(1) performance instead of O(n) linear search
    this.unitToTypeMap = {};
    for (const [type, units] of Object.entries(this.conversions)) {
      if (type === 'temperature') continue; // Handle temperature separately
      for (const unit of Object.keys(units)) {
        this.unitToTypeMap[unit] = type;
      }
    }
    
    // Load unit scaling rules from data file
    this.unitScalingRules = window.UnitConverterData.UNIT_SCALING_RULES;
  }
  ...
  getUnitType(unit) {
    ...
    if (this.unitToTypeMap[normalizedUnit]) {
      return this.unitToTypeMap[normalizedUnit];
    }
   ...
   * Uses lookup table configuration for maintainable scaling logic
   ...
   * @param {string} sourceUnit - Optional source unit to avoid circular conversions
   * @returns {Object} - {value, unit} with the best unit choice
   */
  getBestUnit(value, unitType, defaultUnit, sourceUnit = null) {
    const typeRules = this.unitScalingRules[unitType];
    if (!typeRules || !typeRules[defaultUnit]) {
      return { value, unit: defaultUnit };
    }
    
    const units = this.conversions[unitType];
    const rules = typeRules[defaultUnit];
    
    for (const rule of rules) {
      // Check if source unit should be excluded from this rule
      if (rule.excludeSource && sourceUnit === rule.excludeSource) {
        continue;
      }
      
      // Check threshold condition based on direction
      const meetsThreshold = rule.direction === 'up' 
      ...
      if (rule.convertFn) {
        ...
        convertedValue = rule.convertFn(value, units);
      } else {
        // Standard conversion using ratio
        convertedValue = value * units[rule.targetUnit];
      }
      
      // Check if converted value meets minimum threshold (for cascading down-scaling)
      if (rule.minValue !== undefined && convertedValue < rule.minValue) {
        continue;
      }
      
      return { value: convertedValue, unit: rule.targetUnit };
    }
